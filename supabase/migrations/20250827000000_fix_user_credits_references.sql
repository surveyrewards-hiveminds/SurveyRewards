-- Fix any remaining references to user_credits table
-- This migration ensures all functions are updated to use credit_transactions instead of user_credits

-- Drop and recreate the process_survey_credit_refunds function to ensure it's clean
DROP FUNCTION IF EXISTS process_survey_credit_refunds();

-- Create a clean version that only uses credit_transactions
...
BEGIN
  ...
  LOOP
    ...
    IF (credits_to_refund + lottery_credits_to_refund) > 0 THEN
      -- Create credit transaction record (NO references to user_credits table)
...
-- Drop and recreate process_survey_lottery_rewards to ensure it's clean
DROP FUNCTION IF EXISTS process_survey_lottery_rewards();
    ...
    LOOP
      ...
      IF tier_winners > 0 THEN
        ...
        FOR participant_record IN 
          ...
          LIMIT tier_winners
        LOOP
          -- Award lottery prize (NO references to user_credits table)
          INSERT INTO credit_transactions (
            user_id,
            transaction_type,
            credit_amount,
...
-- Drop any remaining functions that might reference user_credits
DROP FUNCTION IF EXISTS create_user_credits_for_new_user();

-- Add a comment to document that user_credits table should never be used
COMMENT ON VIEW user_credit_balance IS 'Source of truth for user credit balances. Calculated from credit_transactions table. The user_credits table was permanently removed and should never be referenced.';
