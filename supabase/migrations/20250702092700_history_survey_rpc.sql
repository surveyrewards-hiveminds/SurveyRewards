-- This file contains SQL functions to retrieve user survey history and price range.
...
as $$
  ...
  group by r.id, s.id
  order by
    case when sort_key = 'name' and lower(sort_direction) = 'asc' then s.name end asc,
    case when sort_key = 'name' and lower(sort_direction) = 'desc' then s.name end desc,
    case when sort_key = 'reward_type' and lower(sort_direction) = 'asc' then s.reward_type::text end asc,
    case when sort_key = 'reward_type' and lower(sort_direction) = 'desc' then s.reward_type::text end desc,
    case when sort_key = 'country' and lower(sort_direction) = 'asc' then s.target_countries[1] end asc,
    case when sort_key = 'country' and lower(sort_direction) = 'desc' then s.target_countries[1] end desc,
    case when sort_key = 'created_at' and lower(sort_direction) = 'asc' then s.created_at end asc,
    case when sort_key = 'created_at' and lower(sort_direction) = 'desc' then s.created_at end desc,
    case when sort_key = 'answered_at' and lower(sort_direction) = 'asc' then r.submitted_at end asc,
    case when sort_key = 'answered_at' and lower(sort_direction) = 'desc' then r.submitted_at end desc,
    -- fallback
    r.submitted_at desc
  limit limit_count offset offset_count;
$$;

-- Function to get the price range of surveys in the user's history
